...
export type SettingsCollection = 'access' | 'activity' | 'fields' | 'asset-types' | 'tags' | 'conversations' | 'notifications' | null;
...
export function formatSettingsRouteHref(state: SettingsRouteState): string {
  ...
  if ((state.settingsLevel === 'tenant' || state.settingsLevel === 'inventory') && state.tenantId) {
    ...
    if (state.settingsCollection && (state.settingsCollection !== 'notifications' || state.settingsLevel === 'inventory') && (state.settingsCollection !== 'conversations' || state.settingsLevel === 'tenant')) {
