...
const profile = { id: 'model', tenantId: 'home', capability: 'language_inference', lifecycleState: 'enabled',
...
describe('conversation model choices', () => {
  it('keeps enabled language profiles and exposes only choice fields', async () => {
    const choices = await repository([profile, { ...profile, id: 'speech', capability: 'speech_to_text' },
      { ...profile, id: 'archived', lifecycleState: 'archived' }]).list('home');
