...
const definition: WorkflowDefinition = { name: 'Home', providerProfileId: null, instructions: '',
...
describe('workflow editor', () => {
  ...
  it('saves edited settings without mutating the loaded revision', async () => {
    ...
    expect(document.querySelector<HTMLButtonElement>('#provider-model')!.disabled).toBe(false);
    expect(saves[0]).not.toHaveProperty('steps');
  });
