<script lang="ts">
 ...
 import {expirationDateLabel} from '$lib/application/expirationPresentation';
 ...
 let label=$derived(expiration ? expirationDateLabel(expiration,context,new Date(),locale) : '');
</script>
{#if expiration}<span class="expiration-label" class:warning class:expired={warning && expired}>{#if warning}{#if expired}<TriangleAlert size={14} aria-hidden="true" />{:else}<Clock size={14} aria-hidden="true" />{/if}{/if}<span>{label}</span></span>{/if}
<style>
