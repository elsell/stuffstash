...
func LoadThumbnails() (ThumbnailConfig, error) {
	cfg := ThumbnailConfig{WorkerEnabled: true, Concurrency: 1, MaxAttempts: 5, PollInterval: time.Second, LeaseDuration: 90 * time.Second, ProcessingTimeout: time.Minute, PublicationTimeout: 15 * time.Second, RetryBase: 5 * time.Second, RetryMax: 5 * time.Minute}
	if value, exists := os.LookupEnv("STUFF_STASH_THUMBNAIL_WORKER_ENABLED"); exists {
		parsed, err := strconv.ParseBool(value)
		if err != nil {
			return cfg, errors.New("invalid thumbnail WORKER_ENABLED")
		}
		cfg.WorkerEnabled = parsed
	}
	for _, field := range []struct {
		name   string
		target *int
	}{{"CONCURRENCY", &cfg.Concurrency}, {"MAX_ATTEMPTS", &cfg.MaxAttempts}} {
	...
	for _, field := range []struct {
		...
		{"POLL_INTERVAL", &cfg.PollInterval}, {"LEASE_DURATION", &cfg.LeaseDuration}, {"PROCESSING_TIMEOUT", &cfg.ProcessingTimeout},
