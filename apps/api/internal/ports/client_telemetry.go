...
const (
	ClientSurfaceApplication ClientSurface = "application"
...
func (m ClientMeasurement) Valid() bool {
	return !math.IsNaN(m.DurationMS) && !math.IsInf(m.DurationMS, 0) && m.DurationMS >= 0 && m.DurationMS <= 60000 &&
		...
		slices.Contains([]ClientSurface{ClientSurfaceApplication, ClientSurfaceHome, ClientSurfaceList, ClientSurfaceDetail, ClientSurfaceGallery, ClientSurfaceFullscreen, ClientSurfaceUpload}, m.Surface) &&
