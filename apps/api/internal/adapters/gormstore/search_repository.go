...
func (s Store) SearchAssets(ctx context.Context, tenantID tenant.ID, inventoryIDs []inventory.InventoryID, page ports.AssetSearchPageRequest) ([]ports.AssetSearchResult, error) {
	...
	query = s.postgresSearchCandidates(ctx, query, tenantID, inventoryIDValues, page)
	results := []ports.AssetSearchResult{}
	cursor := page.AfterResultKey
	for {
		if err := ctx.Err(); err != nil {
			return nil, err
		}
		var assetModels []assetModel
		batchQuery := searchAfterCursor(query.Session(&gorm.Session{}), cursor).
			Limit(searchCandidateBatchSize)
		if err := batchQuery.Find(&assetModels).Error; err != nil {
			return nil, err
		}
		...
		assetIDValues := make([]string, 0, len(assetModels))
		typeIDs := make([]string, 0, len(assetModels))
		for _, model := range assetModels {
			assetIDs = append(assetIDs, asset.ID(model.ID))
			assetIDValues = append(assetIDValues, model.ID)
			if model.CustomAssetTypeID != nil {
				typeIDs = append(typeIDs, *model.CustomAssetTypeID)
			}
		}
		currentCheckouts, err := s.searchOpenCheckouts(ctx, tenantID, inventoryIDValues, assetIDValues)
		if err != nil {
			return nil, err
		}
		assetTypes, err := s.searchCustomAssetTypes(ctx, tenantID, typeIDs)
		if err != nil {
			return nil, err
		}
		attachments, err := s.searchAttachments(ctx, tenantID, inventoryIDValues, assetIDValues)
		if err != nil {
			return nil, err
		}
		...
		for _, model := range assetModels {
			...
			if !assignedTagsContainAll(assignedTags[item.ID], page.TagIDs) {
				continue
			}
		...
		last := assetModels[len(assetModels)-1]
		cursor = last.InventoryID + ":" + last.ID
		if len(assetModels) < searchCandidateBatchSize {
			break
		}
	}
...
func (s Store) searchOpenCheckouts(ctx context.Context, tenantID tenant.ID, inventoryIDValues []string, assetIDValues []string) (map[string]asset.Checkout, error) {
	var models []assetCheckoutModel
	if err := s.db.WithContext(ctx).
		Where(&assetCheckoutModel{TenantID: tenantID.String(), State: asset.CheckoutStateOpen.String()}).
		Where(map[string]any{"inventory_id": inventoryIDValues, "asset_id": assetIDValues}).
...
func (s Store) searchCustomAssetTypes(ctx context.Context, tenantID tenant.ID, typeIDs []string) (map[customfield.AssetTypeID]customfield.AssetType, error) {
	if len(typeIDs) == 0 {
		return map[customfield.AssetTypeID]customfield.AssetType{}, nil
	}
	var models []customAssetTypeModel
	if err := s.db.WithContext(ctx).Where(&customAssetTypeModel{TenantID: tenantID.String()}).Where(map[string]any{"id": typeIDs}).Find(&models).Error; err != nil {
		return nil, err
	}
...
func (s Store) searchAttachments(ctx context.Context, tenantID tenant.ID, inventoryIDValues []string, assetIDValues []string) (map[string][]media.Attachment, error) {
