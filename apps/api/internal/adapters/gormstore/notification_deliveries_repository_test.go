...
func TestNotificationDeliveryAtomicPublicationAndFencing(t *testing.T) {
	...
	record := auditRecord(t, "device-audit", scope.TenantID, scope.InventoryID, audit.ActionNotificationCreated)
	...
	if _, _, err := store.InsertNotificationWithDeliveries(ctx, notice, []ports.NotificationDelivery{delivery}, conflictingAudit); err == nil {
