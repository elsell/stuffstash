...
func TestGoogleConversationNativeToolsPreserveSignedHistory(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(contents) == 1 {
			_, _ = w.Write([]byte(`{"candidates":[{"content":{"role":"model","parts":[{"thought":true,"text":"private reasoning","thoughtSignature":"fixture-signature"},{"functionCall":{"name":"search","args":{"query":"chemicals"}}}]}}]}`))
		} else {
			_, _ = w.Write([]byte(`{"candidates":[{"content":{"role":"model","parts":[{"text":"Yes, you have chemicals, including Acetone."}]}}]}`))
		}
	}))
	defer server.Close()
