...
func TestBatchRejectsInvalidVariantSetBeforePublication(t *testing.T) {
	for _, variants := range [][]media.ThumbnailVariant{nil, {media.ThumbnailVariantSmall, media.ThumbnailVariantSmall}, {media.ThumbnailVariantSmall, "invalid"}} {
		count := 0
		err := (StandardImageProcessor{}).CreateThumbnails(context.Background(), ports.ImageDerivativesRequest{ContentType: media.ContentTypePNG, Content: []byte("source"), Variants: variants}, func(media.ThumbnailVariant, ports.ImageDerivative) error { count++; return nil })
		if err == nil || count != 0 {
