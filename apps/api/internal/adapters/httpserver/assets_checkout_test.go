...
func TestAssetCheckoutEndpointsRejectUnauthorizedAndCrossScopeAccess(t *testing.T) {
	...
	readCases := []struct {
		...
		{name: "asset detail missing auth", path: assetPath, status: http.StatusUnauthorized, code: "authentication_required", message: "Authentication required."},
		{name: "asset detail malformed auth", path: assetPath, authorization: "Bearer nope", status: http.StatusUnauthorized, code: "authentication_required", message: "Authentication required."},
		{name: "asset list missing auth", path: "/tenants/" + tenantID + "/inventories/" + inventoryID + "/assets?limit=10", status: http.StatusUnauthorized, code: "authentication_required", message: "Authentication required."},
		{name: "asset list malformed auth", path: "/tenants/" + tenantID + "/inventories/" + inventoryID + "/assets?limit=10", authorization: "Bearer nope", status: http.StatusUnauthorized, code: "authentication_required", message: "Authentication required."},
		{name: "search missing auth", path: "/tenants/" + tenantID + "/search/assets?q=Checkout&inventoryId=" + inventoryID + "&checkoutState=checked_out", status: http.StatusUnauthorized, code: "authentication_required", message: "Authentication required."},
		{name: "search malformed auth", path: "/tenants/" + tenantID + "/search/assets?q=Checkout&inventoryId=" + inventoryID + "&checkoutState=checked_out", authorization: "Bearer nope", status: http.StatusUnauthorized, code: "authentication_required", message: "Authentication required."},
	}
