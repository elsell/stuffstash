...
func RegisterArchive(api huma.API, application app.App) {
	huma.Patch(api, "/tenants/{tenantId}/inventories/{inventoryId}/assets/{assetId}/archive", func(ctx context.Context, input *dto.UpdateAssetLifecycleInput) (*dto.UpdateAssetLifecycleOutput, error) {
		principal, err := shared.Authenticate(ctx, application, input.Authorization)
		if err != nil {
			return nil, err
		}

		item, err := application.ArchiveAsset(ctx, app.UpdateAssetLifecycleInput{
			Principal:   principal,
		...
		return &dto.UpdateAssetLifecycleOutput{
			Body: shared.SuccessEnvelope[dto.AssetResponse]{
				Data: mapper.AssetToResponse(item, nil, nil, nil),
