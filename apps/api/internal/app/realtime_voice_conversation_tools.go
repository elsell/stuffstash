...
func (e *realtimeConversationTools) ExecuteConversationTool(ctx context.Context, call ports.AgentToolCall) (ports.ConversationToolOutcome, error) {
	...
	if call.Name == RealtimeVoiceToolSearchAuthorizedAssets {
		if _, err := parseRealtimeVoiceSearchArgs(call.Arguments); err == nil {
			arguments := make(map[string]any, len(call.Arguments)+1)
			for key, value := range call.Arguments {
				arguments[key] = value
			}
			arguments["limit"] = realtimeVoiceToolMaxResults
...
func realtimeConversationReadTools() []ports.ConversationToolDefinition {
	return []ports.ConversationToolDefinition{
		...
		realtimeConversationExpirationTool(),
		realtimeConversationExpirationCalendarTool(),
		{Name: RealtimeVoiceToolSearchAuthorizedAssets, Description: "Search authorized inventory names, descriptions and tags. The query is plain text: every whitespace-separated term must occur somewhere in the same item; terms may match different fields or tags. Boolean operators such as OR are not supported. For alternatives, issue separate search tool calls, which may be requested together. Results already include parentTitle and containmentPath for recorded locations; you can answer a location question directly from these results without fetching detail. An empty lexical search does not cover differently named items or related categories. Returns up to 20 candidates so competing places remain visible. Never infer absence or uniqueness from hasMore=true; use a parent-scoped list when needed. Use distinct category synonyms for discovery questions; relevance must be assessed from the results.", Parameters: json.RawMessage(`{"type":"object","properties":{"query":{"type":"string"},"lifecycleState":{"type":"string","enum":["active","archived","all"]}},"required":["query"],"additionalProperties":false}`)},
