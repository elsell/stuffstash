...
func buildRealtimeVoiceProviders(ctx context.Context, cfg config.Config) (ports.SpeechToTextProvider, ports.ConversationModel, ports.TextToSpeechProvider, error) {
...
func buildRealtimeVoiceProvidersWithTokenSource(cfg config.Config, tokenSource oauth2.TokenSource) (ports.SpeechToTextProvider, ports.ConversationModel, ports.TextToSpeechProvider, error) {
	if !cfg.VoiceGoogleEnabled {
		if cfg.VoiceDevFakeEnabled {
			return voice.DevFakeSpeechToText{}, voice.DevFakeLanguageInference{}, voice.DevFakeTextToSpeech{}, nil
		}
...
func buildRealtimeVoiceProviderResolver(cfg config.Config, repositories repositories, vault ports.ProviderCredentialVault, stt ports.SpeechToTextProvider, lm ports.ConversationModel, tts ports.TextToSpeechProvider) ports.RealtimeVoiceProviderResolver {
	if stt != nil && lm != nil && tts != nil {
		return staticRealtimeVoiceProviderResolver{providers: ports.RealtimeVoiceProviderSet{
			SpeechToText:      stt,
			ConversationModel: lm,
			TextToSpeech:      tts,
		}}
	}
