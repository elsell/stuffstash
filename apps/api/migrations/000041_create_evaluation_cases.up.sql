CREATE TABLE conversation_evaluation_cases (
...
CREATE TABLE conversation_evaluation_case_revisions (
 ...
 CONSTRAINT evaluation_case_revision_sequence UNIQUE (tenant_id,case_id,number),
 FOREIGN KEY (tenant_id,case_id) REFERENCES conversation_evaluation_cases(tenant_id,id) ON UPDATE CASCADE ON DELETE RESTRICT
);
...
ALTER TABLE audit_records ADD CONSTRAINT chk_audit_records_action CHECK (action IN (
  ...
  'conversation_workflow.revision_created','conversation_workflow.activated','conversation_evaluation_case.revision_created',
