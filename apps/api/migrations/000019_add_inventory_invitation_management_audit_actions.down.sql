...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM audit_records
    WHERE action IN ('inventory_invitation.listed','inventory_invitation.expiration_updated')
  ) THEN
    RAISE EXCEPTION 'cannot roll back inventory invitation management audit actions while audit records exist';
  END IF;
END $$;
