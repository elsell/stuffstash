...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM audit_records
    WHERE action IN ('undoable_operation.undone','undoable_operation.redone')
       OR target_type = 'undoable_operation'
  ) THEN
    RAISE EXCEPTION 'cannot roll back undoable operation audit actions while audit records exist';
  END IF;
END $$;

DROP TABLE undoable_operations;
