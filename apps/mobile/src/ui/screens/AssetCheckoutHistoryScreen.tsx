...
export function AssetCheckoutHistorySheetRouteScreen({ assetCheckoutHistoryQuery, assetCoreQuery, assetId }: {
  ...
  const state: AssetCheckoutHistorySheetState = first
    ...
    : history.isError || inventory.isError ? { status: 'error', assetTitle, message: 'Checkout history could not be loaded.' }
      : { status: 'loading', assetTitle };
  const retry = () => { void (inventory.isError ? inventory.refetch() : history.refetch()); };
  return <SafeAreaView style={{ flex: 1, backgroundColor: palette.surface }} edges={['left', 'right', 'bottom']}>
    ...
    <AssetCheckoutHistorySheet state={state} onClose={() => router.back()} footer={<>
      ...
      {history.hasNextPage ? <Pressable accessibilityLabel="Load older checkouts" accessibilityRole="button" disabled={history.isFetching} onPress={() => { if (!history.isFetching) void history.fetchNextPage(); }}>
