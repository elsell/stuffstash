...
export function VoiceResultRail({ references, railKey, onOpen }: { readonly references: readonly VoiceResponseArtifact[]; readonly railKey: string; readonly onOpen: OpenReference }) {
  ...
  const move = (next: number) => {
    ...
    scroll.current?.scrollTo({ x: next * cardWidth, animated: false });
  };
  return <View style={styles.rail}>
    <ScrollView ref={scroll} onLayout={event => setViewportWidth(event.nativeEvent.layout.width)} contentContainerStyle={{ paddingRight: Math.max(0, viewportWidth - cardWidth) }} horizontal snapToInterval={cardWidth} decelerationRate="fast" showsHorizontalScrollIndicator={false}
      contentOffset={{ x: current * cardWidth, y: 0 }}
...
function VoiceResultCard({ reference, onOpen }: { readonly reference: VoiceResponseArtifact; readonly onOpen: OpenReference }) {
  ...
  return <AssetCard asset={detail.data} density="row" style={{ backgroundColor: colors.surfaceMuted, borderRadius: radius.lg }} showTags={false} onPress={() => onOpen(reference)} onParentLocationPress={parent => onOpen({ type: 'asset_reference', assetId: parent.id, title: parent.title, assetKind: 'location' })} />;
}
