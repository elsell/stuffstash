...
it.each(['current', 'returned', 'background'] as const)('owns device Settings failure and retry in the %s context', async context => {
  ...
  try {
    await h.render(<NotificationSettingsScreen tenantId="tenant" inventoryId="inventory" session={session}
      assetTypesQuery={{ async execute() { return []; } }} pushSession={{ async enable() { return 'denied'; } }} onBack={() => {}} onNavigate={() => {}} />);
    await h.settle();
    await h.press(h.byLabel('Set up alerts on this device')); await h.settle();
    const writesBeforeLaunch = writes;
    await h.press(h.byLabel('Open notification system settings'));
    expect(h.byLabel('Open notification system settings')?.props.disabled).toBe(true);
    await h.press(h.byLabel('Open notification system settings'));
    ...
    await h.press(h.byLabel('Open notification system settings')); await h.settle();
