...
it('clears compact selection search without losing staged tags and resets it on Back', async () => {
 ...
 type Search = { placement: string; onFocus: () => void; onChangeText: (event: { nativeEvent: { text: string } }) => void; onClose: () => void };
 ...
 try {
  ...
  await h.run(() => {search().onFocus(); search().onChangeText({nativeEvent:{text:'Travel'}});});
  expect(h.byLabel('Medicine')).toBeUndefined();
  await h.run(()=>search().onClose());
  expect(h.byLabel('Medicine')?.props.accessibilityState.checked).toBe(true);
  await h.run(() => {search().onFocus(); search().onChangeText({nativeEvent:{text:'missing'}});});
