  ...
  return <>
    ...
    {props.fieldType === 'enum' ? <View style={styles.formRow}><Text style={styles.label}>Options</Text>{props.enumOptions.map(option => props.persistedEnumOptions.includes(option) || !props.canMutate
      ...
      : <NativeCommandButton key={option} label={`Remove ${option}`} disabled={disabled}
          onPress={() => { if (!disabled) props.onEnumOptions(props.enumOptions.filter(value => value !== option)); }} />)}{props.enumOptions.length === 0 ? <Text accessibilityLiveRegion="polite" style={styles.validationText}>Add at least one option.</Text> : null}{props.canMutate ? <View style={styles.inline}><AppTextInput editable={!disabled} accessibilityLabel="New enum option" onChangeText={props.onNewOption} placeholder="Add option" style={styles.input} value={props.newOption} /><Pressable accessibilityRole="button" disabled={disabled} accessibilityState={{ disabled }} onPress={() => { if (disabled) return; const next = suggestedCustomizationKey(props.newOption); if (next && !props.enumOptions.includes(next)) props.onEnumOptions([...props.enumOptions, next]); props.onNewOption(''); }} style={styles.inlineAction}><Text style={styles.inlineActionText}>Add</Text></Pressable></View> : null}</View> : null}
    <View style={styles.formRow}>{props.mode === 'edit' ? <Text style={styles.label}>Applies to</Text> : null}{props.mode === 'edit' ? props.applicability === 'all_assets' ? <Text style={styles.lockedValue}>All assets</Text> : <><Text style={styles.lockedValue}>Selected asset types</Text>{props.canMutate ? <Choice disabled={disabled} label="Expand to all assets" onPress={() => props.onApplicability('all_assets')} selected={false} /> : null}</> : <SingleChoicePicker disabled={disabled} label="Applies to" onChange={props.onApplicability} options={[{ label: 'All assets', value: 'all_assets' }, { label: 'Selected asset types', value: 'custom_asset_types' }]} value={props.applicability} />}</View>
...
function Choice({ disabled, label, onPress, selected }: { readonly disabled: boolean; readonly label: string; readonly onPress: () => void; readonly selected: boolean }) { const colors = useAppearancePalette(); const styles = createStyles(colors); return <Pressable accessibilityRole="radio" accessibilityState={{ checked: selected, disabled }} disabled={disabled} onPress={() => { if (!disabled) onPress(); }} style={[styles.choice, selected && styles.choiceSelected, disabled && styles.disabled]}>{selected ? <Check color={colors.action} size={17} /> : null}<Text style={styles.choiceText}>{label}</Text></Pressable>; }
