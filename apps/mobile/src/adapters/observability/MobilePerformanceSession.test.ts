...
it('sends product latency with authentication and disposes without recursive delivery', async () => {
  ...
  const session = createMobilePerformanceSession({
    platform: 'ios', enabled: true, baseUrl: 'https://api.example.test', tokenProvider: () => 'private-session', ...time,
    fetch: async input => {
      ...
      if (new URL(request.url).pathname === '/client-telemetry') {
        ...
        expect(await request.json()).toEqual({ measurements: [{ platform: 'ios', operation: 'request', surface: 'application', variant: 'none', outcome: 'success', durationMs: 25 }] });
        delivered();
...
it('disabled sessions preserve fetch identity and never schedule image delivery', () => {
  ...
  const session = createMobilePerformanceSession({ platform: 'android', enabled: false, baseUrl: 'https://api.example.test', tokenProvider: () => null, fetch: fetchImpl, ...time });
...
it('clears pending images on session disposal and ignores late image completions', () => {
  const time = runtime();
  const session = createMobilePerformanceSession({ platform: 'ios', enabled: true, baseUrl: 'https://api.example.test', tokenProvider: () => null, fetch: async () => new Response(), ...time });
