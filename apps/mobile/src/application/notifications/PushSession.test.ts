...
it('binds registration to the authenticated principal and cleans it before disconnecting',async()=>{
 ...
 const setup=new PushSetup({async requestPermission(){return true;},async nativeToken(){return {transport:'apns',token:'abcd'};}},{
...
it('does not disconnect when cleanup fails or mutate after identity resolution is cancelled',async()=>{
 ...
 const setup=new PushSetup({async requestPermission(){throw new Error('unexpected');},async nativeToken(){throw new Error('unexpected');}}, {
