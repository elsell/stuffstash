...
it('leaves preferences and registration untouched after permission denial',async()=>{
 const calls:string[]=[];
 const setup=new PushSetup({async requestPermission(){return false;},async nativeToken(){throw new Error('not allowed');}}, {async installationId(){return 'phone';},async remember(){calls.push('remember');},async list(){return [];},async forget(){calls.push('forget');}}, {async getByInstallation(){throw new Error('not allowed');},async register(){throw new Error('not allowed');},async revoke(){throw new Error('not allowed');}}, {record(){}});
...
it('journals before registration and preserves cleanup metadata after an uncertain failure',async()=>{
 const calls:string[]=[];
 const setup=new PushSetup({async requestPermission(){return true;},async nativeToken(){return {transport:'apns',token:'abab'};}}, {async installationId(){return 'phone';},async remember(){calls.push('remember');},async list(){return [];},async forget(){calls.push('forget');}}, {async getByInstallation(){throw new NotificationFailure('not-found');},async register(){calls.push('register');throw new NotificationFailure('unavailable');},async revoke(){throw new Error('not allowed');}}, {record(){}});
...
it('refreshes revision on cleanup conflicts and leaves other users records alone',async()=>{
 const calls:string[]=[];
 let revision=1;
 const setup=new PushSetup({async requestPermission(){return false;},async nativeToken(){throw new Error('unused');}}, {
...
it('enables delivery only after journaling and registration succeed',async()=>{
 const calls:string[]=[];
 const setup=new PushSetup({async requestPermission(){return true;},async nativeToken(){return {transport:'apns',token:'abab'};}}, {
