...
import { redactUnsafeVoiceStructuredText, redactUnsafeVoiceText } from './VoiceTextSafety';
...
export function isValidVoiceActionPlanProposal(proposal: VoiceActionPlanProposal): boolean {
  ...
  for (const command of proposal.commands) {
    ...
    if (command.expiration !== undefined && (!isAssetExpiration(command.expiration) || (command.kind !== 'create_asset' && command.kind !== 'create_location'))) return false;
...
function safeBoundedActionPlanText(value: string, maxLength: number): string {
  const normalized = redactUnsafeVoiceStructuredText(value)
    .replace(/\s+/g, ' ')
    .trim();
  if (normalized.length <= maxLength) {
    return normalized;
