...
cat > "$workdir/apps/mobile/src/ui/screens/LegacyModal.tsx" <<'EOF'
import { Modal } from 'react-native';
export function LegacyModal() { return <Modal visible />; }
EOF

if "$checker" "$workdir/apps/mobile/src" >"$workdir/output" 2>&1; then
  echo "expected a raw product Modal to fail the mobile UI structural check" >&2
  exit 1
fi

grep -F "LegacyModal.tsx" "$workdir/output" >/dev/null
