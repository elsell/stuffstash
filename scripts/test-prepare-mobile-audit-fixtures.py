...
class FixtureRouteIsolationTests(unittest.TestCase):
    ...
    def test_notification_journey_uses_production_tabs_and_real_detail(self):
        ...
        self.assertIn("NotificationJourneyFixture", target.read_text())
        self.assertFalse((self.routes / "assets/[assetId]/index.tsx").exists())
        self.assertIn("BrowseFilterJourneyDetail", (self.routes / "(tabs)/(home,search)/assets/[assetId]/index.tsx").read_text())
        for layout in self.tab_layouts:
            self.assertEqual((self.routes / layout).read_text(), f"production layout {layout}\n")

    def test_inventory_collection_uses_production_tabs_and_real_screen_fixture(self):
        self.assertEqual(self.run_script(AUDIT_TEST_CASE="inventory-clearance").returncode, 0)
        target = self.routes / "(tabs)/(home,search)/assets/index.tsx"
        self.assertIn("InventoryCollectionFixture", target.read_text())
        self.assertFalse((self.routes / "assets/[assetId]/index.tsx").exists())
        for layout in self.tab_layouts:
            self.assertEqual((self.routes / layout).read_text(), f"production layout {layout}\n")
    ...
    def test_retains_production_routes_and_installs_only_fixture_exports(self):
        ...
        self.assertEqual({p.name for p in self.routes.iterdir()} - {"search.tsx", "browse-filters.tsx", "expiration-filters.tsx", "expiration.tsx", "audit-browse-journey.tsx", "assets", "audit-edit-journey.tsx", "audit-android-header-composition.tsx", "audit-notifications.tsx", "audit-notification-target.tsx", "audit-managed-search.tsx", "audit-contents-search-preconfigured.tsx", "audit-tabs", "audit-invitation.tsx", "audit-customization-editor.tsx", "audit-settings-readback.tsx", "audit-settings-commands.tsx", "voice.tsx", "voice-plan-location.tsx", "audit-native-search-placement.tsx", "add.tsx", "settings.tsx"},
        ...
        self.assertEqual({str(p.relative_to(self.routes)) for p in (self.routes / "audit-tabs").rglob("*.tsx")},
                         {*[p.replace("(tabs)/", "audit-tabs/", 1) for p in self.tab_layouts], "audit-tabs/(home)/index.tsx", "audit-tabs/(search)/search.tsx", "audit-tabs/(home,search)/assets/[assetId]/index.tsx", "audit-tabs/(home,search)/settings/inventory/tags/[resourceId].tsx", "audit-tabs/(home,search)/expiration.tsx", "audit-tabs/(home,search)/settings/sharing.tsx", "audit-tabs/(home,search)/assets/footer-clearance.tsx"})
