...
| Add/Edit selection | All four selection lifecycle cases pass on phone/iPad in35903554297, with exact-value timing retained. [Evidence](evidence/selection-lifecycle-359035-results.txt). | Visual review rejects clipped Cancel new place in iPad creation; focused native creation form correction pending. General input responsiveness remains unproven. |
| Move | 35907123046 phone connected move/detail/reopen passes; revised rows have coherent spacing and full-row targets. Creation stops at exact-text observation although final capture contains Audit. | Preserve exact input and downstream creation checks with the established bounded observation; iPad and creation visual acceptance remain open. |
| Move Here | 35907123046 phone confirms Retry is behind the header after moving status outside List. Typed status/retry content now belongs inside the native list. | 51 source tests and Android recovery pass; corrected phone/iPad native recovery remains required; [diagnosis](evidence/move-layout-359071-phone-results.txt). |
