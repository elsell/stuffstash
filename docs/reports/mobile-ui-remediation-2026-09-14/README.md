...
| Add/Edit selection | 35914746387 iPad passes all four cases; focused creation capture has clear native commands and visible name. Phone passes three cases but destination search opens without keyboard; final capture hides initial context. | Direct iOS scroll ownership and measured creation clearance now implemented; nine focused source tests, TypeScript, structural/fixture checks and critic pass. Initial context, search keyboard and creation clearance require the corrected phone/iPad run. |
| Move | 35911803207 connected move/detail/reopen passes on phone and iPad; iPad creation passes. Phone creation starts under the header. | Reserve measured header space for the creation form; corrected native creation and whole-layout acceptance remain open. |
| Move Here | 35911803207 phone rejected-command/retry and iPad suggestions recovery pass. Remaining cases stop in search cleanup: task Cancel selected on phone; keyboard disappears between iPad snapshots. | Scope clear to the search field, observe dismissal settling, and retain command/retry/return checks. Whole-layout acceptance remains open. |
