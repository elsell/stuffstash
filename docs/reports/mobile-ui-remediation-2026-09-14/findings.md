...
Runtime totals and named test results below are scoped evidence. In run351214,
iPad results are log-only because result export was interrupted; they do not imply
inspected screenshots or whole-surface acceptance. See [iPad evidence limit](native-ipad-351214.md).
...
| M15 | Unsaved enum options cannot be removed before saving | Implemented; scoped native passes | Run351214 phone/iPad logs pass testDraftOptionRemovalPreservesSavedOptions: actual draft removal disappears, saved Existing option remains, and saved removal is unavailable. This does not establish every enum-editor layout or assistive mode. |
...
| M17 | iPad onboarding stretches the form across the display with excessive separation from its action | Implemented; landscape native scenario passes | Run35247151136 iPad job105290143896 passes testOnboardingAdaptsToLandscape: actual landscape frame, visible address and reachable Connect. Centered600-point source constraint remains; this scenario does not assert exact form width or enlarged-text layout. Phone landscape is intentionally unsupported and skipped. |
...
| M20 | Onboarding keyboard does not dismiss with downward content drag | Scoped native passes on phone/iPad | Run35247151136 onboarding jobs105290143544/105290143896 pass testConnectionHelpAndKeyboardKeepActionsReachable, including a downward drag near the scroll margin, keyboard disappearance, reachable Connect and preserved full URL. The iPad inside-form comparison also passes; its phone skip is expected. This verifies dismissal, not keyboard-open command clearance (M35). |
...
| M22 | Appearance uses navigation for three flat choices | Scoped native passes on phone/iPad | Run35247151136 testAppearanceUsesMenuWithoutNavigation selects Dark through the actual native menu, observes the changed value, and keeps the same return command reachable. Storage-failure rollback retains source coverage; it is not exercised by this native scenario. |
...
| M30 | Provider credential/prompt drafts remain editable during Save | Implemented; native recovery passes; pending-state coverage remains | Run35247151136 passes credential Save recovery and prompt Save recovery/Discard on phone/iPad: native header commands, failed-save recovery, retry/return, and prompt draft retention. Pending-save input freezing and secret clearing retain the17 source tests; these are not asserted by those native scenarios. |
...
| M35 | Onboarding Connect is largely covered by the phone keyboard | Corrected fixture passes phone/iPad/Android; no product layout change needed | Production setup replaces the navigation stack. Removing the audit-only header/in-flow observer restores that viewport. Native35818325756 passes full keyboard-open Connect clearance, one-tap exact URL submission and keyboard Go on both iOS devices (2/2 each); reviewed captures retained. Android APK8db5032592a7e18b5f9fab0dfd2a724d9cf4d53aa1ec1329fe3a0ac3c686aea6 passes bounds[63,1188][1017,1313] above IME top1517 and one-tap exact submission. The prior header-bearing failure was not a shipped layout defect. Wider device/font configurations remain separate. |

| M36 | Type reminder mode uses custom choice rows for three flat values | Scoped native passes on phone/iPad | Run35247151136 testReminderModeUsesMenuWithoutNavigation chooses Custom in place, reveals Before expiration, then chooses Use defaults and removes that field. Failed persistence and Discard retain source coverage; broader assistive/adaptation checks remain. |
...
| M45 | History commands retain custom controls despite available native adapters | Implemented; scoped native passes on phone/iPad | Run35247151136 testCheckoutHistoryTextBoundsPaginationAndDismissal verifies text bounds, sheet presentation (explicit expansion assertion on phone only), asset-name retry, older-page loading and native Close. It does not cover History reversal, every error state or enlarged text. Source tests/checks and critic evidence remain below. |
...
with the original failure retained in native-menu-358154-results.csv. Focused
