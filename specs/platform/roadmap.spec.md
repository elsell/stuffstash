...
Provider pinning now has a versioned credential-vault port that returns immutable record identity and owned secret bytes from the same repository read, with scope/retirement validation shared by legacy reads. Its failing-tests-first pass and code review are complete; CI is pending. Effective provider fingerprints still need runtime configuration and explicit server-ADC version coverage, followed by snapshot resolver and worker integration.
