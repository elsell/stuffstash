...
Provider pinning now has a versioned credential-vault port that returns immutable record identity and owned secret bytes from the same repository read, with scope/retirement validation shared by legacy reads. Its failing-tests-first pass and code review are complete; CI is pending. Effective provider fingerprints now cover canonical profile configuration, immutable credential ID and factory runtime identity; Google includes operator ADC project/region/quota and the environment-backed credential revision. This pass has source review and regression tests, with CI pending. Snapshot resolver and worker integration must use the same identity computation and reject drift; these are not wired yet.
