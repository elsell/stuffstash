...
The current branch has workflow draft revision persistence/API wiring, operator limits, tag-evidence preservation and grounded response recovery. CI run 33979131161 passed for commit 5fb456441, including exact proposal assertions and atomic tenant workflow selection. Shared model execution budgets, retries and separate step instructions passed CI run 33979531979 at 1fb203627. Selected revisions now prepare pinned per-step providers for production voice, with lazy shared execution budgets and no dependency on unused default language models; this integration passed CI run 33979848024 at 9ff5480a1. Pinned evidence-round budgets, precise-first versus expanded retrieval and candidate-capacity clarification passed CI run 33980187309 at 2c1014c7e. Negotiated answer continuity and explicit additional-item creation passed CI run 33981024508 at e11777b12. Coverage includes retained context, workflow follow-up limits, exhausted-budget availability, fresh-request intent separation, revoked access, grounded user evidence, mandatory discovery even with multiple existing matches, and approval-backed new identity creation. The earlier exhaustion-test fixture audit dependency is corrected. Evaluation fixture definitions, deterministic outcome assertions and lossless completed-response/persisted-command projection exist. Projection rejects external identities, undisplayed location evidence, empty responses and unsupported semantic arguments; its tests-only baseline failed CI run 33981148410 as expected, and implementation passed CI run 33981329849 at c9ce3fcfb. The isolated text executor now composes fresh fixture storage with the production conversation loop and pinned workflow, counts provider calls, projects unapproved proposals, propagates cancellation and independently detects fixture/checkout mutations; it passed code-critic review and CI run 33981735515 at 1e6f32af2 after tests-only run 33981550364 failed on the absent executor as expected. It explicitly does not validate speech. Versioned evaluation case storage is implemented in memory and GORM with migration 41, scoped immutable revisions, transactional audit/CAS, rich-definition round trips and bounded pagination; it passed code-critic review and CI run 33982114508 at 0467727de. The evaluation-case API tests-only baseline failed run 33982397188 because routes were absent. Tenant-configure-protected create, append, latest/history reads and scoped pagination are now implemented, including fail-closed read auditing; implementation CI is pending. CI now regenerates the Go API contract and publishes it for review, closing a stale-contract validation gap without local builds. These components are not yet connected to an execution worker or web UI. Next integrate the shared workflow executor, immutable case/run persistence, provider selection, activation gates and web editing/evaluation. Live ADC tests and the TestFlight/GitOps release gates remain outstanding; do not treat domain-only evaluation tests as voice acceptance evidence.
