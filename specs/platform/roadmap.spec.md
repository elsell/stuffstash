...
The current branch has workflow draft revision persistence/API wiring, operator limits, tag-evidence preservation and grounded response recovery. CI run 33979131161 passed for commit 5fb456441, including exact proposal assertions and atomic tenant workflow selection. Shared model execution budgets, retries and separate step instructions passed CI run 33979531979 at 1fb203627. Selected revisions now prepare pinned per-step providers for production voice, with lazy shared execution budgets and no dependency on unused default language models; this integration passed CI run 33979848024 at 9ff5480a1. Pinned evidence-round budgets, precise-first versus expanded retrieval and candidate-capacity clarification passed CI run 33980187309 at 2c1014c7e. Negotiated answer continuity is implemented across the server and mobile client pending CI, including retained context, workflow follow-up limits, exhausted-budget availability, fresh-request intent separation and revoked-access coverage. CI run 33980566334 reached a missing audit adapter in the new exhaustion test fixture; the fixture is corrected for the next run. Explicit additional-item creation passed code-critic review and is pending CI, with grounded user evidence, mandatory discovery even with multiple existing matches, and approval-backed new identity creation. Evaluation fixture definitions and deterministic outcome assertions exist; they are not yet connected to an execution worker or web UI. Next integrate the shared workflow executor, immutable case/run persistence, provider selection, activation gates and web editing/evaluation. Live ADC tests and the TestFlight/GitOps release gates remain outstanding; do not treat domain-only evaluation tests as voice acceptance evidence.
