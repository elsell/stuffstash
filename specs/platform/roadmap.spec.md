...
The image-performance workstream now follows `specs/platform/observability.spec.md`: instrument traces, metrics, logs, and profiling; capture the unchanged baseline; implement measured media/mobile/web improvements; report a like-for-like comparison. Work is isolated on `codex/media-observability`. Grafana management access is verified; Cloud ingestion credentials and the target benchmark environment are pending. Local disk space is insufficient for builds, so preserve CI/remote validation.
