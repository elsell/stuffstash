...
Evaluation-run query services now require tenant configure permission, scope cursors and reads, and audit before returning data. Memory/SQLite/PostgreSQL conformance covers read-audit persistence through migration 44; application tests cover denied access, audit failure, tenant isolation and 100/101-row pagination. Source review passed; query CI is pending. HTTP DTO/mapper/routes and adversarial boundary tests are the next integration step.
