...
- Realtime voice loop control now treats expected malformed model tool calls and final-response contract violations as recoverable graph-like states: it emits safe tool failure events, returns sanitized tool error results to the model, requests final-only recovery when the tool budget is exhausted, and completes with a safe spoken response instead of dropping the socket when text-to-speech is available.
