...
- Mobile asset detail for containers and locations must show contained assets directly below the workspace metadata:
  ...
  - Container and location workspaces should elevate spatial actions over generic asset maintenance: `Add item here` should be the primary contained-section action and should remain close to the contained-assets heading, while generic `Edit`, `Move`, and `Add photos` actions remain available as quiet maintenance controls after the contained-assets workspace.
  - Contained-assets headings should anchor the spatial context to the current asset, such as `Inside Garage cabinet`, with the count as secondary text. The heading and spatial actions should make the section read as the continuation of the detail page rather than a separate browse module.
