...
- Mobile asset detail must be an asset workspace, not a read-only card or a pile of unrelated buttons:
  ...
  - Asset cards and recent-asset cards must not show last-updated metadata or photo-readiness chips in the visible card body. When an asset is checked out, the checked-out status chip belongs as a compact top-right overlay on the asset image, not next to the location breadcrumb row or in the secondary metadata area.
  - Home recent and checked-out entries, selected-inventory lists, Search results, and location-scoped lists must render through the same shared asset-card component. The component may expose standard and compact density, optional tag visibility, and an optional footer action, but card chrome, media and placeholder treatment, title, checked-out overlay, parent breadcrumbs, and tag presentation must have one implementation. Compact density may omit long-form description and search-match metadata when those details would crowd the horizontal Home ticker.
