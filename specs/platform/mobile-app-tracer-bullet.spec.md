...
- Mobile asset detail for containers and locations must show contained assets directly below the workspace metadata:
  ...
  - Container and location workspaces should elevate spatial actions over generic asset maintenance: `Add item here` should be the primary contained-section action and should remain close to the contained-assets heading, while generic `Edit`, `Move`, and `Photos` actions remain available in the asset workspace.
