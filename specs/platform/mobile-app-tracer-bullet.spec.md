...
- `Settings` must remain available as a native stack route from a trailing account affordance on Home rather than consuming a bottom-tab slot. The account affordance may use the authenticated principal's avatar or a familiar person symbol and must expose an explicit `Open account and settings` accessibility label. The Settings overview, tenant/inventory drill-ins, and production custom field, custom asset type, and tag management behavior must follow `specs/platform/client-settings-management.spec.md`.
...
- The first `Home` surface must be a calm, activity-focused native dashboard:
  ...
  - A `Checked out` attention section that appears only when assets are checked out and uses compact shared asset entries with direct Return actions instead of large image-dominant cards. A quiet empty sentence must not reserve a full section when there is nothing checked out. Its `View all` shortcut must open Browse List for all active checked-out assets, ordered recently changed, clearing any previous query, tags or kind filter. Home collection shortcuts replace refinements rather than merging unrelated earlier Browse criteria.
...
- The Home inventory switcher must use the space left by the actual visible header actions, up to a readable 320-point width. A viewer with only Profile must not inherit the tighter width needed for Add/Notifications/Profile. Preserve native group margins and at least a 44-point switcher target; truncate only after using the available bounded space. Verify one-action and three-action layouts at normal text size without action overlap.
...
- The Home recent-assets section must open asset detail routes. Its `See all` action must navigate to the existing Browse tab for the selected inventory, showing the List surface with all active assets ordered by recently changed. It must explicitly clear previous query, tags, kind and availability refinements; ordinary tab switching continues to preserve Browse context. Do not create a second reduced-function asset grid for this Home shortcut. Verify entry from a previously refined Browse state and return through asset details with persistent tabs.
...
- Mobile asset detail must be an asset workspace, not a read-only card or a pile of unrelated buttons:
  ...
  - Photo-free Browse grid cards must prioritize identity over a blank media area. A grid row whose assets all have confirmed absence of photos uses compact kind/status bands that grow with their content. If a row includes a photo-bearing or unresolved asset, its peers retain equal media space so titles align and compact cards do not stretch into blank panels. Determine this from the current column count, including after resizing. Cards with photos retain their image presentation, and pending/unavailable thumbnail resolution for an asset known to have photos must retain stable media space. Carry this distinction as typed presentation data from asset metadata; do not infer it from translated labels or the temporary absence of a thumbnail URI. Compact row consumers retain their existing thumbnail geometry. Verify photo-free, photo-bearing, mixed and loading grids, checked-out status, and retained scroll before accepting the change. This is a project density decision, not a claim that Apple mandates one card layout.
  ...
  - Repeated asset and Place cards must use tonal surface separation with at most a semantic hairline outline. Light appearance must not draw a dark control border around cards. Touch-down feedback for cards, card media, card text regions, and quiet utility controls must be a subtle opacity response that preserves the idle background, border color, border width, corner radius, and layout. A transient press must never add a heavy outline or reuse a filled selected-state background. Filled primary actions may retain their semantic pressed fill, and selected controls must keep selection visually distinct from transient press feedback.
  ...
  - Lifecycle, audit, destructive, and other less-frequent secondary actions must live behind a platform-native contextual menu anchored to the trailing overflow control instead of occupying the visible utility or spatial action rows. An overflow control must not open a modal action sheet or alert-based action list. Its native menu anchor must be mounted and measured as the actual 44-point interaction target when the header becomes interactive so the first intentional tap opens the menu; a first tap must not be consumed only to initialize or focus the native host.
  - The overflow must expose lifecycle actions when available and a `History` action for read-authorized assets.
  - The overflow must describe itself as general asset actions rather than lifecycle-only actions when it also contains history commands. Logically related history, lifecycle, and irreversible actions must remain ordered and visually distinguishable, and permanent deletion must retain native destructive treatment and explicit confirmation. Contextual menus dismiss by selecting an action or tapping outside and must not add a redundant `Cancel` row.
  - Normal routed asset detail and embedded Map asset detail must render the same shared menu item model through platform-native menu adapters so ordering, authorization, destructive roles, and callbacks cannot drift between entry points. Routed iOS detail must use the native navigation-toolbar menu adapter; embedded content and platforms without native header items may use the shared anchored menu adapter. The pinned native-stack adapter version must satisfy the installed Expo Router's declared range so native header items cannot be silently discarded by an older navigation adapter.
  ...
  - After appearance preference hydration, React Native surfaces must receive one concrete semantic palette matching the resolved preference and current contrast setting on their first visible frame. They must not depend on an asynchronously settling native dynamic-color trait that can temporarily mix light surfaces with dark borders or text. Native navigation chrome must remain synchronized through the platform appearance override, and System changes must still recompute the concrete palette immediately.
...
- Mobile asset detail History browsing must be read-only except for the explicit authorized `Revert change` command, and it must remain scoped to the current inventory and asset:
  ...
  - History defaults to `changes` and provides an accessible platform-native filter menu for `Changes` and `All events`, anchored to the visible filter control rather than presented as a modal action list. The current selection must carry native selected-state semantics. Read audit records remain available in `All events` but must not bury state changes in the default view.
  ...
  - History loading, pagination, refresh, and reversal failures must never render arbitrary transport `Error.message` text. Known authorization, missing-resource, and conflict states use curated copy; unknown failures use calm generic recovery copy that cannot disclose URLs or diagnostics.
...
- Mobile asset movement must be a dedicated placement picker:
  ...
  - Creating a missing destination is a command, distinct from selecting an existing row. Use the shared native command adapter with a complete accessible action label; keep placement/help text outside its tappable label. Preserve the existing creation/move lock, selected result and failed draft recovery.
...
- Mobile asset detail photo management must use the existing authorized attachment APIs:
  ...
  - System photo selection must distinguish cancellation from unsupported formats. If any selected image has a known unsupported MIME type, reject that new selection with a readable message naming supported JPEG, PNG and WebP formats; do not silently discard images or accept an unexplained subset. Existing draft photos remain unchanged. This applies to Add, existing assets and voice-plan photo drafts, without conversion or relabeling unsupported bytes.
  - Camera/library source choices belong to the focused visit and resource that opened the chooser. Add, asset detail and voice-plan flows must ignore a retained source callback after blur, return or resource replacement, before opening a system picker. Opening a fresh chooser after return remains supported. This does not cancel an already-started system selection merely because the picker itself is presented.
  - The Add draft photo tile must expose the action name `Add photos` independently of its icon and supplementary camera/library hint.
  - The shared photo-source chooser requires an explicit current-visit predicate and checks it before presentation and source acceptance. New consumers must establish ownership rather than relying on a default always-current callback.
...
- Browse must be a combined list, search, and map inventory surface for the selected inventory:
  ...
  - Browse refinement controls must follow native search hierarchy: the search field comes first, followed by one compact refinement toolbar. Asset Type is a list refinement rather than persistent Browse navigation, so `All`, `Places`, `Containers`, and `Items` must live inside Filters as a mutually exclusive `Type` group using the shared native platform segmented control. `All` is the default. A non-default Type must increment the applied-filter count, remain visible as a removable applied-filter token, and preserve its typed durable route and deep-link state. Cancelling the sheet must preserve the previously applied Type; applying commits the draft Type; `Clear all` and Reset restore `All`.
  ...
  - List refinements must be disclosed through one compact `Filters` control. `Filters` and `Sort` must form one coherent refinement toolbar: both triggers use equal 44-by-44-point platform-native bordered controls, share one visual treatment and aligned baseline, and avoid redundant filled card wrappers around the native controls. The Filter trigger must use the platform filter-lines symbol and the Sort trigger must use the platform bidirectional vertical sort-arrows symbol; an ellipsis is reserved for general overflow and must not represent sorting. The visual treatment must remain light enough that the controls read as secondary to search. The `Filters` control must show an active-filter count only when non-default Type, lifecycle, availability, or tag refinements are applied. The count must appear as a compact badge that does not alter the control width and must be included in its accessible label. Default state must not be narrated as a list of active filters.
  - Applied secondary filters must remain visible as removable, user-labeled tokens when the filter surface is closed. These tokens are compact, custom removable pills because they represent an applied state plus a remove action rather than a platform selection control. Their visible pill should be approximately 32 points high while a transparent containing target preserves at least 44 points of interaction height; the visual pill must not be inflated to the full hit-target height. They must retain sufficient contrast and consistent spacing. Tag filters must use display names rather than only a tag count. `Clear all` must be available when more than one secondary filter is active.
  - The expanded filter surface must use a native sheet or equivalent platform-appropriate temporary surface with separate `Type`, `Status`, `Availability`, and `Tags` groups. It must maintain draft filter state until the user applies it, support reset and cancellation, and keep each meaningful control target at least 44 by 44 points. The small mutually exclusive `Type`, `Status`, and `Availability` sets must use the shared native segmented control without redundant checkmarks. Because iOS does not define a native multi-select chip control, Tags must use alphabetically ordered, native-feeling selectable list rows with explicit checkmarks, tag-color indicators, Dynamic Type-safe labels, and checkbox accessibility semantics instead of a custom chip grid.
  ...
  - Sort is not a filter. Browse must expose sort through a separate compact platform-native menu anchored to its visible control, never a modal action sheet or alert-based action list. The native menu trigger must participate directly in the refinement toolbar rather than sit inside a custom filled button wrapper. The current sort must carry native selected-state semantics. `Recently changed` is the default user-facing label. Until the API exposes a more meaningful alternative such as alphabetical order, the deterministic ID order must be labeled `Default order`, not `Stable`.
  ...
  - Mobile route parsing must preserve every supported Browse entry parameter, including List/Map surface, Type, query, selected tag IDs, lifecycle state, checkout state, and sort. `surface=list|map` is typed durable route state with `list` as the omitted/default value; changing the surface must update the route without duplicating result requests, and a Map deep link must open Map directly. Home `View all`, tag chips, and deep links must resolve through the same typed applied-filter state.
  ...
  - Browse interactive controls and cards must provide visible but restrained pressed states. Card presses must use opacity without changing the idle border or fill; transient press feedback must not be confused with selected filter state. Selected filters and Type must not rely on color alone, and List/Map must expose selected tab or segmented-control semantics to assistive technologies.
...
- Browse Map must be a native-feeling horizontal containment map for non-destructive inventory exploration:
  - Map must be presented as its own `Map` tab inside Browse, alongside the normal `List` browse surface. The persistent `List`/`Map` selector must use the shared native platform segmented control, preserve the selected surface in durable Browse route state, and expose the selected state to assistive technologies without a redundant checkmark.
...
- Every editable mobile text input must use the shared project-owned text-input primitive so keyboard behavior, accessibility, and platform conventions remain consistent across onboarding, search, asset workflows, settings, sharing, provider configuration, voice, maps, sheets, and modal editors. Production UI modules must not render React Native `TextInput` directly; test-support adapters may model it at the framework boundary.
- On iOS, the shared input primitive must attach the project-owned native keyboard accessory to every editable single-line and multiline field. The accessory must expose a trailing, at-least-44-point keyboard-dismiss action with a visible platform-appropriate keyboard-down symbol, the accessibility label `Dismiss keyboard`, and button semantics. Activating it must dismiss the keyboard without submitting, mutating, validating, or clearing the focused field.
- The accessory implementation must work inside the native bottom-tab shell, native stacks and sheets, React Native modals, and multiline `TextInput`; React Native `0.83` core `InputAccessoryView` is not sufficient because its documented limitations cover bottom tabs and multiline input. Until those limitations are removed and physically verified, mobile must use the pinned Expo-supported keyboard-controller provider and its keyboard-attached extension behind project-owned components. The extension must attach to every focused editable input independently of the screen's React view hierarchy and use the system-owned keyboard material and adaptive system colors. On iOS 26 and later, the system-native presentation is an inset glass capsule; its controls must remain completely inside that material rather than appear detached at the screen edge. Screens and domain workflows must not import that infrastructure dependency directly.
- A single mounted accessory may serve every input on the current screen or application tree; screens must not render overlapping or duplicated accessory bars when several fields exist. Read-only or disabled text presentations must not summon or attach keyboard controls.
- The application-level accessory host must remain outside normal root layout flow. While the keyboard is closed it must consume no page height, add no bottom inset, and must not lift or shorten bottom tabs, navigation bars, sheets, or screen content.
- Input return-key behavior remains owned by the field workflow. Search, Next, Done, and submit actions must continue to work as specified; the accessory dismissal action is separate and must never synthesize a return-key submission.
- Scrollable mobile form and search surfaces that can contain a focused input must opt into native keyboard dismissal: interactive drag dismissal on iOS and drag dismissal on Android where supported. Taps on actionable controls must remain operable while the keyboard is visible, and scroll dismissal must supplement rather than replace the explicit iOS accessory action.
- Android must preserve its system keyboard-dismiss behavior and must not receive a redundant app-owned accessory bar. Hardware-keyboard, VoiceOver, TalkBack, Dynamic Type, and narrow-screen use must retain access to field labels, values, return-key actions, and the dismissal action where applicable.
...
- Settings must be production-shaped even in the Expo Go validation slice:
  ...
  - Diagnostics must always display locally available connection and application values, even when account or household discovery is pending or fails. Identity values load independently with named loading and recovery states; do not substitute blank IDs or hide local diagnostics behind remote readiness. Retry the failed identity read without discarding available information. Access failures must suppress cached identity values.
  ...
  - Household and inventory settings first-load states must identify what is loading with the existing labeled settings progress row. An unlabeled spinner must not be the only task feedback while the scoped settings query is pending.
...
- Asset History pull-refresh feedback follows the same ownership rule, including asset and history-view identity. A failure from a departed visit or replaced history view must not report over the current task. Cached ordinary history pages remain usable after refresh failure; access failures retain the existing suppression behavior.
- History inline Retry is a read action, not a pull gesture. Disable the retry control while its query is fetching and retain the failure context until recovery, without activating the native pull indicator.
- Asset-detail pull errors belong to the visit/resource that began the refresh. A late failed progressive read must not display a notice after leaving or returning; current-visit failures keep their useful error detail. Independent section recovery and background reconciliation retain their existing query ownership.
- Inventory asset, location content, and location lists must report a failed explicit refresh while retaining previously loaded rows for ordinary connectivity failures. Use the shared notice surface with a clear retry instruction. Feedback belongs to the initiating focused visit/resource; late failures after navigation stay silent. Access failures continue to hide cached rows through the scoped query adapter. Background reads do not trigger pull notices or indicators.
