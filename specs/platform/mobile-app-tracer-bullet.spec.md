...
- Mobile asset detail must be an asset workspace, not a read-only card or a pile of unrelated buttons:
  ...
  - Normal routed asset detail and embedded Map asset detail must render the same menu item model through the same shared native-menu component so ordering, authorization, destructive roles, and callbacks cannot drift between entry points.
...
- Browse must be a combined list, search, and map inventory surface for the selected inventory:
  ...
  - Browse interactive controls and cards must provide visible pressed states. Selected filters and Type must not rely on color alone, and List/Map must expose selected tab or segmented-control semantics to assistive technologies.
