...
- Release automation that opens a self-host image digest update pull request must wait for the pull request's `Required checks` status rollup before merging it. It must allow GitHub time to attach checks to a newly opened pull request, and it must not treat a separate manually dispatched check run as a substitute for the pull request's branch-protection status.
