...
- Release automation that opens a self-host image digest update pull request must wait for the pull request's required branch-protection checks before merging it. It must allow GitHub time to attach required checks to a newly opened pull request, and it must not treat a separate manually dispatched check run as a substitute for the pull request's required status checks.
