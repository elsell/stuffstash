...
- Initial workspace discovery may load identity and tenant access in sequence, but independent inventory-scoped context collections must load concurrently. Selecting another inventory in the same authenticated repository session must reuse the already resolved principal and visible-tenant context instead of refetching `/me` and `/me/tenants`. Session replacement recreates the repository and its permission context.
- Active assets, checked-out summaries, custom types, fields, and tags must load concurrently after inventory discovery. A failure in a secondary schema, tag, or checked-out collection must not erase an otherwise usable inventory surface or asset list.
- Asset detail must render its identity and core metadata after the asset request completes. Attachments and checkout history must load concurrently as independent, labelled states and must not hold the entire detail route behind a single serial loading screen. Attachment metadata must render before protected thumbnail materialization completes.
- Browse query results may be cached by repository identity, tenant, inventory, query, tags, lifecycle, checkout state, scope, sort, surface, page cursor, and page size for the current browser session. Returning to an unchanged query must reuse the resolved result. Mutations that can change a cached result must invalidate the affected inventory's browse entries.
- Replaced Browse, containment-map, asset-detail, attachment, and checkout-history reads must propagate cancellation through the generated client and abort obsolete network work rather than only ignoring late results. Additional route-owned workflows should adopt the same boundary as their generated client operations gain signal support.
- Switching among Home, Browse, Import, and Settings must update route presentation immediately. Unchanged Browse query results may be reused when the user returns during the same repository session.
- Import polling must run only while the import surface is mounted, the document is visible, and at least one job is active. It must prevent overlapping polls and use bounded backoff after failures; a foreground refresh may run when visibility returns.
- Secondary authenticated workflows, including Import and Settings, must be emitted as lazy client chunks rather than being included in the initial workspace chunk. The production build must keep every emitted client JavaScript chunk below 500 kB minified.
- Performance tests must assert request budgets for same-session inventory switching and repeated Browse queries. Delayed controlled fakes must prove independent inventory reads overlap, cancelled Browse work cannot poison re-entry, and asset core presentation does not wait for secondary data. Browser smoke coverage must prove the Home-to-Browse return budget.
