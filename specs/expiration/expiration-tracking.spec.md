...
## Approved cross-platform usability revision (2026-09-11)

Implement the complete expiration UX audit in `docs/reports/expiration-ui-ux-audit-2026-09-11.md`. This supersedes earlier precision-draft switching and clear-active-only interaction details.

- Add uses a bounded large native sheet with a stable completion action and one keyboard-aware scrolling form. Type and expiration are compact labeled rows. Optional description and searchable tags must not make completion unreachable. Selection uses checkmarks and accessible state, not color alone.
- Contextual native date entry and month/year selection preserve calendar precision. Exact-to-month retains its month/year for review; month-to-exact requires an explicit day choice. Removing expiration clears all precision drafts. Cancellation preserves the prior committed draft. Type changes must not silently discard a recorded draft date. Web uses equivalent accessible platform controls.
