...
The tag-query case uses an enabled Supplies type and a separate Medicine tag, with matching upcoming, expired, distant-date and undated items, plus an untagged upcoming item of the same type. Only the upcoming tagged item belongs in the answer's asset references. This prevents the single-item fixture from concealing confusion between tags/types or upcoming/expired/unknown dates.
...
### Repetitions and broader audio regression

Three further Flash expiration runs passed all 24 deterministic cases in 125.79 seconds (`/tmp/expiration-live-native-repeat.log`). Structured event files in `/tmp/expiration-live-evidence-native-repeat` retain the final repetition because the harness reuses scenario paths; earlier repetitions remain in the log. These are repeated execution results, not independent semantic judgments.

The existing eight-case recorded-audio interaction corpus passed six cases but failed two (`/tmp/expiration-native-audio.log`; `/tmp/expiration-native-audio-evidence`). Existing/missing coffee-counter creates, missing-bedroom create, additional-item create, ambiguous-move clarification and color follow-up passed their assertions. Moving an existing drill incorrectly proposed `update_asset` with `expiration:null`, despite a move summary. Creating a toolbox and moving the drill into it repeatedly made the same command-kind mistake and exhausted the budget. No proposal was approved and the fixture inventory remained unchanged. This is a release blocker: command selection must remain correct for ordinary inventory requests after adding expiration commands. These synthesized recordings exercise real speech recognition and synthesis, not a physical microphone.

### Native command schema projection

Applying a flat command decoding shape to native declarations corrected both move cases while preserving all eight expiration cases (`/tmp/expiration-native-flat.log`, 43.19 seconds). The full audio corpus then passed eight cases (`/tmp/expiration-native-flat-audio-full.log`, 56.76 seconds). Unlike the rejected JSON-envelope experiment, this uses native required function calls. The shared catalog and strict application parsers retain their original constraints.

Critic review identified redundant nested alternatives in the projection. A regression test failed before the fix; the merger now flattens and deduplicates alternatives without branching over top-level description differences. The final full API suite and structural checks passed. Critic review confirmed the issue resolved with no further actionable finding.

After cleanup, Flash passed all sixteen live cases in 118.07 seconds (`/tmp/expiration-native-flat-final.log`; events `/tmp/expiration-native-flat-final-evidence`). Primary-agent review confirmed moves use `move_asset` with the intended existing parent or earlier toolbox command, while create requests retain the intended containment. Follow-up answered the drill's recorded red color; ambiguous move asked Office versus Kitchen; location response preserved Bin 8 / Hall closet and September 18, 2026. The ambiguous date prompted clarification without proposing a write, though its suggested interpretations included unnecessary month/year guesses and could be clearer. No approval or inventory mutation occurred.

The corresponding Flash-Lite expiration cases passed four of eight: all three dated creates and ambiguity failed (`/tmp/expiration-native-flat-lite.log`). Its audio cases passed three of eight: existing move, dependent move and color follow-up; total run time was 219.64 seconds. Based on this comparison, the server fallback and newly recommended mobile language profile now use Flash. Explicit configured model choices remain unchanged. Config/bootstrap tests, the mobile profile repository tests and typecheck, and structural checks passed remotely; critic review found no confirmed issues. The accepted projection addresses demonstrated Flash command-selection failures; it does not establish reliable Flash-Lite expiration handling, physical-device acceptance, or completed release.

### Mixed tag-query acceptance

The first tag run passed but lacked an untagged control; critic review correctly identified that it did not prove tag filtering. After adding an untagged upcoming Supplies item, the final Flash run passed in 44.83 seconds (`/tmp/expiration-tag-query-final.log`; `/tmp/expiration-tag-query-final-evidence`). The response named only Tylenol, Hall closet / Bin 8 and September 18, 2026. Its asset reference excluded the untagged, expired, distant-date and undated controls. Primary-agent semantic review and structural checks passed; critic confirmed the fixture correction. The latency is a concern, not a responsiveness success claim. This remains one bounded fixture, not pagination or physical-device acceptance.
