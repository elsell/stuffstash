...
- Compose candidate IDs as parameter-bound GORM subqueries combined with `UNION ALL`, keeping index-friendly ASCII predicates separate from conservative Unicode/custom-field fallback predicates. Membership is tested against that union within the authorized asset scope; duplicates never duplicate results. This avoids a single unindexed fallback branch disabling indexed scalar/attachment lookup.
