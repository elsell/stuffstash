...
- Compose candidate IDs as parameter-bound GORM subqueries combined with deduplicating `UNION`, keeping index-friendly ASCII predicates separate from conservative Unicode/custom-field fallback predicates. Membership is tested against that union within the authorized asset scope; candidate IDs are deduplicated before outer membership evaluation. This avoids a single unindexed fallback branch disabling indexed scalar/attachment lookup.
