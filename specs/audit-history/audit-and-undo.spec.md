...
- Asset create, archive, and restore application commands must expose one operation-aware result contract. Asset-only compatibility facades that discard the operation identifier are forbidden; internal callers that need only the asset must still consume the operation-aware result explicitly.
- The Go structural check must reject asset-only `CreateAsset`, `ArchiveAsset`, or `RestoreAsset` methods on the application `App` facade or asset application `Service`, without rejecting repository-port methods that persist assets.
...
- Applying the operation and refreshing the broader inventory are separate outcomes. Once the API applies Undo or Redo, a later refresh failure must not relabel it as an operation failure or remove the inverse Redo/Undo action. The returned asset must be reconciled immediately, the successful compensation must remain announced, and a separate dismissible `warning` notification with infinite duration must explain that the visible inventory may be stale. An authentication failure during that refresh must enter the normal session-expired flow.
- Undo/Redo failure copy may include a reason only when the web adapter marks it safe for users. Internal, transport, and server-failure details must be replaced with bounded generic recovery copy.
