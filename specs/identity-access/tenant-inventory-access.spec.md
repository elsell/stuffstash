...
- Private-LAN HTTP invitation links use the browser acceptance experience for local development and must not generate iOS associated-domain or Android verified app-link declarations. Production HTTPS origins may provide verified native app linking only when that origin is compiled into a deployment-specific mobile build; the general self-host-capable store build cannot claim arbitrary instance domains.
...
- Before offering copy or share, clients must validate the complete creation link and its tenant, inventory and invitation identities. A configured invitation origin pins the returned link to that origin. The general mobile build, which has no compiled-in invitation origin, may accept a canonical HTTPS link directly returned by its connected authenticated API: that server mints the token and configures the browser acceptance origin. This exception does not apply to incoming links or change web-client origin policy. An unconfigured client may accept loopback HTTP only with explicit local-development opt-in; a configured private-LAN HTTP origin likewise requires that opt-in. Public HTTP, credentials and invalid link shapes remain rejected. See the outbound-creation boundary in mobile-oidc-authentication.spec.md.
