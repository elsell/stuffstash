...
Tool definitions identify a response tool that can deliver any natural answer, including a question or an answer without cards. When such a tool is available, the Google adapter requests schema-constrained function output (`ANY` mode) across the entire supplied catalog, as documented in Google's [function calling modes](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/multimodal/function-calling#function-calling-modes). The model chooses whether to read, propose or answer; the application does not select a semantic stage. No extra inference is needed to format a final answer. Without a response tool, the adapter preserves automatic text/tool choice. The core port continues to accept plain text from providers that support it.
