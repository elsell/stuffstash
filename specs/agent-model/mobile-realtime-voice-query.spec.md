...
Provider translation may canonicalize only fields whose relevance is determined entirely by an already-validated discriminator. A non-`create` operation drops `newAssetKind`; `search` and `search_again` drop resolutions; and `finish` drops search and vocabulary requests. Read-kind normalization drops search probes from `list_inventory` and all ID-targeted reads, and drops `visibleAssetId` from discovery reads. Only `create` and `move` have destination references: every other operation drops destination path and kinds plus extraneous destination reads or resolutions; when its sole initial read incorrectly labels the subject mention as `destination.0`, translation may relabel that same mention as `subject`. This canonicalization is not a retry, heuristic repair, or alternate interpretation: it must never change the selected decision, intent kind, operation, mentions, destination segment text, candidate IDs, evidence, or rationale. All remaining project validation still applies. Provider translation and application policy may derive role-specific no-candidate status only when the candidate list is empty: an unresolved requested destination or proposed new create subject is `missing`, while an unresolved subject for a read or existing-resource change is `absent`. They must not convert a non-empty ambiguous resolution, unsupported intent, or an ungrounded resolution into a successful plan.
