...
Provider translation may canonicalize only fields whose relevance is determined entirely by an already-validated discriminator. A non-`create` operation drops `newAssetKind`; `search` and `search_again` drop resolutions; and `finish` drops search and vocabulary requests. This canonicalization is not a retry, heuristic repair, or alternate interpretation: it must never change the selected decision, intent kind, operation, mentions, destination path or kinds, read arguments, candidate IDs, evidence, or rationale. All remaining project validation still applies. The application may likewise derive role-specific no-candidate status from anchored intent and executed discovery evidence: an unresolved requested destination or proposed new create subject is `missing`, while an unresolved source required for an existing-resource change is `absent`. It must not convert ambiguity, unsupported intent, or an ungrounded resolution into a successful plan.
