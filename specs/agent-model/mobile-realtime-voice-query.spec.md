...
When executed discovery already proves every original create-or-move reference as either a sole normalized exact match or zero compatible candidates, the application may deterministically finish the anchored original intent even if the evidence-assessment output requests redundant reads or drifts to a different finish intent. Destination candidates whose base kind conflicts with the requested destination kind do not prevent zero-compatible-candidate completion. This completion may use only authorized observations and recorded discovery coverage; it must not promote fuzzy same-kind candidates, invent a destination, or override a valid transcript-grounded destination repair.
