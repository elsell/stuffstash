...
Gemini language inference must request structured final responses through provider-native structured output controls instead of relying only on prompt wording. The adapter must set JSON response MIME type and a response schema for the final-response envelope whenever a Gemini turn may produce a textual final response, including tool-capable turns where the model may choose not to call another tool. The prompt may briefly tell the model to follow the provided final-response schema, but it must not duplicate the full JSON schema or maintain a separate hand-written final JSON example that can drift from the actual provider schema.
