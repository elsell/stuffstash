...
2. Execute the validated reads through application services and return bounded observations for one evidence assessment, with materially different additional rounds only within the pinned workflow allowance.
...
- Allow no more than the pinned workflow evidence-round allowance; the default remains two and the structural ceiling is eight.
...
- intent kind, one typed operation from the supported read and change operation taxonomy, and a typed request shape of `single_target`, `collection_target`, or `compound`;
...
The default evidence budget is two read rounds. A selected workflow supplies its own bounded allowance under `conversation-workflows.spec.md`. A round may execute multiple narrow reads for different semantic references or complementary probes, but the application must deduplicate equivalent queries, reject repeated work as progress, bound total requests and result sizes, and stop on cancellation or timeout. Typed follow-up reads such as asset detail, contents, asset history, or checkout history may consume a remaining round when required by the operation. The provider may request an additional round within its allowance only when it identifies a genuinely new probe or required typed read; it must not spend the budget repeating a failed query in different punctuation or word order.

The evidence-assessment turn may repair an initially incomplete or inside-out destination path for `create` or `move` when rereading the transcript reveals an explicit enclosing place or container that was omitted or ordered incorrectly. A repair must preserve request shape, intent kind, operation, subject mention, proposed asset kind, and details; preserve every original destination mention exactly once; and only add explicit transcript-grounded destination mentions or reorder the existing mentions. An original segment that remains at the same position must retain its destination kind. A segment whose position changes because the chain was repaired may be reclassified, but its fresh read kind hint must agree with the repaired destination kind. It must transition to the remaining evidence round and issue fresh reference-scoped discovery for every repaired destination reference. The application must discard earlier destination-scoped evidence whose positional reference changed while retaining subject evidence and the complete tool audit trail. The repair may never drop a named destination, change the requested operation, or extend the pinned evidence-round budget.
...
- separating one operation on one target from an operation over a collection and from a request that combines multiple operations;
...
Request shape is an orthogonal semantic classification, not a transcript keyword rule. `single_target` means one requested operation on one subject or proposed new asset. `collection_target` means one operation applies to an explicit set, category, universal quantification, or otherwise unbounded collection. `compound` means the request asks for two or more operations, including sequential operations on the same subject. Collection reads remain supported where their operation defines a collection result. A collection-targeted change and every compound request are unsupported in the first production command surface. Provider translation must derive canonical operation `unsupported` for those shapes before the application can execute inventory reads or compile a plan. It must never silently retain one supported mutation from a compound request.
...
Collection intent does not imply an unfiltered inventory list. `list_inventory` reads are appropriate for an explicit whole-inventory request or a base-kind browse. A named semantic category, remembered group, tag-like phrase, or household classification must use bounded `search_assets` hypotheses so unrelated inventory rows are not included merely because they share a base kind. The model may use the category label, synonyms, and several distinct likely category members as retrieval hypotheses; those guesses remain reads, not inventory facts. The terminal collection may contain only candidates returned for that reference by those category-preserving reads.

Every typed asset-discovery read must carry a project-owned lifecycle scope of `active`, `archived`, or `all`; omitted scope means `active`. The initial subject discovery for archive and restore must use `all` so application policy can distinguish a needed transition from the already-archived or already-active no-op. A lifecycle-status question may use `all` when it genuinely spans both states; other operations must prefer the narrowest applicable scope. Provider translation may derive the archive/restore subject scope from the anchored operation. The application must pass this validated scope to the authorized search or list query and preserve it in read evidence. A terminal resolution is not grounded when its supporting discovery evidence came from a lifecycle scope that could not contain the resolved candidate. The provider must not use lifecycle scope to bypass authorization or infer that an omitted result exists in another scope.
...
- request-shape anchoring so collection-targeted changes and compound requests cannot become partial executable plans;
...
Canonical intent is anchored from the initial interpretation. Evidence turns must preserve request shape, kind, operation, subject mention, create kind, and destination path and kinds. The application also preserves the initial `details` value and discards any evidence-turn rewrite of that field, because checkout facts, history summaries, or observation text are evidence rather than new user-authored command details. A later model turn cannot add provider-observed text to executable checkout or return arguments.

The application may terminate an unnecessary `search_again` decision without another provider call only when every required reference has executed discovery coverage and every resolution follows deterministic grounding policy: a sole exact normalized-title candidate is strong, no exact normalized-title candidate for a proposed create subject is missing even when discovery returned non-exact distractors, zero compatible candidates for a requested destination are missing, zero candidates for an existing source are absent, and every destination after the first missing segment is missing. For a nested destination, same-kind candidates whose recorded parent differs from the preceding sole exact destination are incompatible with that requested chain and do not prevent missing completion; a fuzzy same-kind candidate under the correct parent remains model-owned. Exact-title comparison may ignore one leading conversational determiner or possessive such as `my`, `the`, `a`, or `an` on the user's mention while retaining the unmodified comparison as an alternative; it must not apply stemming, edit distance, category inference, or other fuzzy matching. Multiple exact create-subject candidates, any non-exact candidate for an existing source or chain-compatible requested destination, ambiguity, unsupported intent, missing coverage, or unseen ID keeps resolution with the model or falls forward safely. Treating a non-exact create-subject result as a distractor is safe because the application still proposes rather than executes the new asset and never reuses the distractor's ID. This exact-or-zero completion is bounded policy, not a fallback search, heuristic match, or model-authored command path.
...
The provider-native schema should constrain the object shape, required fields, phase-valid decisions, typed request shape, and bounded enum vocabulary without duplicating every semantic invariant as conditional or combinatorial schema branches. Candidate cardinality, total request and resolution budgets, reference completeness, and phase-transition semantics remain mandatory application validation. This keeps constrained decoding within provider state limits while preserving the stricter project-owned contract. The initial Gemini turn normally permits only `search`, requires no resolutions, and prohibits unseen `visibleAssetId` values. After one initial semantic inference, the application may immediately finalize a product-owned `unsupported_action` with zero inventory reads whenever canonical operation is `unsupported`, including a `compound` shape or collection-targeted change. This is the normal typed contract path, not a local phrase parser or fallback.

Provider translation may canonicalize only fields whose relevance is determined entirely by an already-validated discriminator. Typed request shape deterministically changes a collection-targeted mutation or compound request to operation `unsupported`; otherwise the selected operation is preserved. The resulting operation deterministically derives the redundant intent kind: read operations are `read`, supported mutations are `change`, and `unsupported` is `unsupported`. A non-`create` operation drops `newAssetKind`; `search` and `search_again` drop resolutions; and `finish` drops search and vocabulary requests. Read-kind normalization drops search probes from `list_inventory` and all ID-targeted reads, and drops `visibleAssetId` from discovery reads. It may remove exact normalized duplicates within a search-probe array, exact duplicate read requests after that normalization, and exact duplicate candidate IDs within one resolution; these operations preserve the same hypothesis and candidate sets and must still leave a structurally valid non-empty value where required. Provider-native array bounds may be used only when the configured provider accepts them within its constrained-decoding state budget; project-owned validation always enforces the bounds after decoding. The initial schema may require at least one read request because the initial provider decision is `search`; application policy may discard those requests when the canonical shape immediately establishes unsupported intent. Only `create` and `move` have destination references: every other operation drops destination path and kinds plus extraneous destination reads or resolutions; when its sole initial read incorrectly labels the subject mention as `destination.0`, translation may relabel that same mention as `subject`. This canonicalization is not a retry, heuristic repair, or alternate interpretation: it must never change request shape, mentions, destination segment text, candidate set, evidence, or rationale. All remaining project validation still applies. Provider translation and application policy may derive role-specific no-candidate status only when the candidate list is empty: an unresolved requested destination or proposed new create subject is `missing`, while an unresolved subject for a read or existing-resource change is `absent`. They must not convert a non-empty ambiguous resolution, unsupported intent, or an ungrounded resolution into a successful plan.
...
Final read responses must answer the semantic question in concise household language rather than narrating search mechanics. The application must first construct a typed grounded response brief from canonical intent, resolution status, and authorized observations. A project-owned response-generator port then realizes that brief as spoken and display text. The response generator may control wording, grammar, contractions, emphasis, and compact summarization, but it may not choose the response kind, add facts, change confidence, create a clarification state, propose or execute commands, or introduce an asset or location absent from the brief.

The grounded response brief must contain only bounded presentation facts needed for the answer: response kind, operation, subject mention, calibrated confidence, answer mode, safe titles, safe containment paths, and safe fact summaries already authorized for the request. It must not contain internal IDs, raw tool calls or results, prompts, model responses, authorization data, audit metadata, credentials, provider details, or hidden inventory data. User-authored strings in the brief are untrusted data and must be delimited as such in provider prompts.

Response generation is a normal production stage, not a fallback or an alternate agent loop. Provider output must use a strict project-owned schema containing only spoken and display text; model-reported fact IDs or provenance claims are not accepted as grounding evidence. The application must validate the spoken and display channels independently for length, safety, response-kind semantics, every required grounded anchor, typed lifecycle or checkout polarity where relevant, and the operation-specific answer shape before speech synthesis. A valid display response must never compensate for an invalid spoken response, and a lexical anchor inside an explicit contradiction or negation does not satisfy grounding. Current-state checkout and lifecycle polarity must not reject grounded past events in history answers merely because the asset is now available or active. In particular, a locate answer must express a location relation and name a location or container supplied by the brief; a contents answer must name every supplied direct content; a broad inventory answer that contains item findings must name every supplied item while allowing standalone place findings to be omitted or used naturally as location context, and an inventory answer with no item findings must name every supplied finding; detail, existence, history, checkout, and no-op answers must retain their supplied subject and safe fact anchors without reversing the supplied state; a not-found answer must retain the requested subject and express absence without inventing a location; and only a clarification brief may produce a question. Invalid response-generation output must fail safely through the provider-stage error contract and must not fall back to deterministic factual prose.

Every grounded response brief must be realizable within the 500-byte spoken and 1000-byte display limits. Before generation, the application must select a deterministic bounded presentation subset by semantic priority and text budget, prioritizing item findings ahead of optional place context for broad inventory answers, bound containment paths to the useful immediate context, and retain at least one bounded safe history or checkout fact when such evidence exists. The brief carries only a truncation marker, not a raw result count; when truncated, both channels must naturally disclose that more authorized results or history exist. The model may summarize or group the selected findings, but it may not silently omit a selected required item or direct content. This presentation bound does not change investigation evidence, action-plan grounding, authorization, or the underlying query result.

The response-generator dependency must be explicit at application and provider-profile composition boundaries. Static construction receives it separately from the investigation interface, while a provider-profile factory returns a language provider that satisfies both project-owned interfaces. Composition must fail when either capability is absent; runtime type assertions and silent option no-ops are not valid dependency wiring.

A collection-shaped locate request must state where the matching items are, using their authorized containment paths. When the only grounded match is a plausibly category-like container or location, the brief may present that resource as the likely place using calibrated confidence; it must not imply that every requested item was individually observed. Broad inventory lists and contents questions may summarize what the user has or what a place contains. User-facing speech and display text must not expose implementation-oriented result language such as "visible match", "candidate", "resolution", "tool result", or raw result counts. Multiple locations should be summarized compactly without discarding material location differences.

The production cutover must compare generated realization against the former deterministic renderer using the same grounded briefs. The comparison must evaluate semantic validity, grounded title and location retention, calibrated uncertainty, absence of implementation vocabulary, question-kind correctness, naturalness, and added generation latency. It must use semantic-family fixtures and predicates rather than requiring frozen exact utterances. After the comparison establishes equal or better semantic correctness and better product language, the deterministic factual renderer and any runtime A/B switch must be removed; a test-only baseline may remain solely to preserve the historical comparison.
...
New items and containers compile to `create_asset`; true locations compile to `create_location`. Existing-asset commands compile only from a single strong or plausible grounded subject. A plausible non-exact subject may be shown as an explicit calibrated assumption, but a plausible non-exact destination must not silently become an executable parent for a write; it becomes a structured clarification unless the application can present the candidate mapping explicitly in the review contract. Lifecycle and custody operations become product-owned no-op answers when already satisfied. The compiler must reject ambiguity, missing existing sources, broken containment chains, unsupported operations, and any attempt to use provider evidence text as command content.
...
When executed discovery already proves every original create-or-move reference as either a sole normalized exact match or a safely missing reference, the application may deterministically finish the anchored original intent even if the evidence-assessment output requests redundant reads or drifts to a different finish intent. A proposed create subject is safely missing when no exact normalized-title result exists, even if discovery returned non-exact distractors; the distractors are not selected or reused. A requested destination is safely missing only when it has zero compatible candidates. Destination candidates whose base kind conflicts with the requested destination kind do not prevent zero-compatible-candidate completion, and a nested candidate whose recorded parent conflicts with the preceding sole exact destination is likewise incompatible with that chain. This completion may use only authorized observations and recorded discovery coverage; it must not promote fuzzy same-kind destinations, invent a destination, or override a valid transcript-grounded destination repair.
...
Create interpretation distinguishes `record` (resolve an existing item before proposing a new record) from `additional` (the user explicitly requests another physical instance). The empty legacy creation mode means `record`. Only create may carry a creation mode. Additional creation requires a short `creationEvidence` quote (at most 200 runes) from the current user utterance or retained user conversation turns; assistant text and inventory labels cannot supply it. Application validation verifies the quote is present, while the configured model interprets whether the quoted wording actually expresses an additional physical instance. Merely finding a similar name never establishes additional intent. Provider instructions must distinguish newly recording an item from explicitly obtaining or adding another one.

Both modes require authorized subject discovery before planning. For additional creation, existing subject candidates do not identify the new physical instance and do not block its proposal; existing candidates must still be validated before the subject is represented as a new identity. Existing destination resolution and all authorization, approval, audit and execution rules remain unchanged. Interpretation and assessment must preserve creation mode and evidence. Approval says that an additional item will be created and never reuses an existing subject ID. Without additional intent, existing-item protection remains unchanged.
