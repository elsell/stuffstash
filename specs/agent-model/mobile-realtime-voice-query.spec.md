...
When the realtime voice sheet displays a proposed action plan, each visible plan row that represents an item, container, or location may offer an inline `Add photos` action. This action must use the same native photo-selection capability as the Add flow, including camera and photo-library choices, native permissions, supported image MIME types, and in-session image previews. Selected photos are draft UI state scoped to the active proposed plan and command row; they must not be uploaded, persisted, logged, added to diagnostics, sent to the language provider, or attached to an asset until the API exposes a post-execution mapping from reviewed plan commands to the concrete asset IDs created or updated by execution. Existing assets referenced by an approved plan may be eligible for immediate attachment only after a future spec defines the exact authorization, confirmation, and failure-recovery behavior for voice-collected photos.
