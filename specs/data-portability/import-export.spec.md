...
Destructive or interruptive import actions, including cancellation and removing a run from import history, must use an in-place reusable modal dialog shell rather than navigating to a separate confirmation page or hand-rolling a one-off overlay. The dialog must preserve the current import context behind it, support Escape/backdrop dismissal when work is not busy, keep keyboard focus inside the dialog, and show busy progress on the chosen action.
