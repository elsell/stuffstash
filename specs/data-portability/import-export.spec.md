...
Default safe options such as normal network protections should not be surfaced as standalone source facts unless the user changed a risky option or needs to understand a failure. When risky options are shown, labels must describe the actual choice in plain language, such as private-network access or self-signed certificate allowance.
