...
Terminal import failures must retain a safe, phase-specific diagnostic in durable import messages before temporary source credentials are deleted. The diagnostic must identify the operation that failed, distinguish source connection or authentication failure from attachment validation and blob persistence, state whether partial records were kept, and give a concrete recovery action. Raw network errors, provider response bodies, credentials, tokens, internal storage paths, and adapter implementation details must not be persisted or returned.
Failure to establish the attachment-reading session before the first attachment must use a dedicated error code and safe detail rather than the generic `import validation failed` fallback. It must preserve the attachment progress total and the successfully created field, tag, location, and asset counts.
Failure to persist a downloaded attachment in configured blob storage must use a distinct durable error code and safe recovery detail. It must identify the affected attachment by safe source identity without persisting the storage key or raw adapter error.
...
Destructive, interruptive, and explanatory import overlays, including cancellation, removing a run from import history, and issue-detail explanations, must use an in-place reusable modal dialog shell rather than navigating to a separate confirmation page or hand-rolling a one-off overlay. The dialog must preserve the current import context behind it, support Escape/backdrop dismissal when work is not busy, keep keyboard focus inside the dialog, prefer the visible dialog title as its accessible name, and show busy progress on the chosen action when applicable.
...
History status summaries must be aligned, compact controls when they filter or jump the page state. They must expose selected state instead of looking like decorative statistics, and their count/icon alignment must make each summary read as a single clickable control. On narrow screens, status filters must remain a compact horizontal control rail or equivalent compact control surface instead of becoming a tall stack that pushes current work and history rows below the first viewport.
History rows must use user-facing status language and must not require users to know durable-job implementation statuses.
History rows should use `Source` as the first ledger column because import sources are pluggable. Source cells should show the source name, instance identity, and source version when available; method details such as live connection, CSV upload, image options, and default network protections must not compete with the source identity unless they require user attention. The default history row should omit actor attribution and unchanged source options when those details compete with source, status, result counts, and completion-time scanning.
History status filters should only demand visual attention for categories that contain runs. Empty categories may be omitted or visually subordinated, but any visible filter must clearly act like a control and reflect the filtered state.
History ledger column headings for source, status, changed records, and finished time should act as sorting controls on desktop/tablet layouts. The default sort may remain priority/recent-first, but once the user chooses a column, the selected sort and direction must be exposed accessibly.
History rows should avoid repeating the same warning or result state across multiple columns. A warning-level completed import should show one amber issue indicator and a concise completion state instead of repeating `Completed with warnings` next to a warning badge; detailed warning counts and explanations belong in job detail.
...
Import detail must preserve the same severity model as history: warning-only runs may show amber indicators in summary tiles, tabs, issue groups, or row badges, but they should not add a second top-level review callout when the warning count is already visible. Blocking errors, failed imports, and discard-cleanup failures require action-required language and red destructive/error emphasis.
...
The import detail page title should identify the source, such as `Homebox import`, rather than using a generic `Import details` heading. Generic detail labels may remain in route names or accessible descriptions, but the first visible heading should orient users around the data source and instance.
...
Default safe options such as normal network protections should not be surfaced as standalone source facts unless the user changed a risky option or needs to understand a failure. Method labels should use plain user language, such as connected directly to Homebox or CSV upload. When risky options are shown, labels must describe the actual choice in plain language, such as allowing a local/private network address or allowing a self-signed certificate.
Secondary import actions must not imply precision that the UI does not provide. If the audit action opens the inventory-wide activity view rather than a run-filtered audit trail, the label and helper text must say that clearly instead of promising import-specific evidence.
The import detail overview should be a scannable control surface, not a passive wall of metrics. Overview tiles should use icons and semantic color sparingly, and tiles that naturally lead to deeper context, such as warning/error counts or imported-record counts, should be keyboard-accessible controls that switch to the relevant detail section without losing route state.
...
Issue groups that open educational guidance must use action language that describes the user's intent, such as explaining the issue, rather than generic `Details` wording that competes with import-run detail actions.
Import preview plan sections must show compact section counts so users can understand the scale of fields, locations, assets, and photos/files before reading individual rows. If the source API returned only a bounded subset, the UI must label it as a partial list rather than a vague sample, and table accessible labels should describe the content as a plan preview.
...
- Put readiness, blocking status, and re-preview requirements ahead of plan preview rows.
- Show bounded partial lists of planned records and state when only a subset is shown.
...
- When a planned-record list is larger than the default visible page, the UI must provide explicit previous/next pagination or an equivalent bounded navigation pattern and must state the visible range. Ambiguous copy such as `shown` without range or total context is not sufficient.
...
The web UI must keep the resource summary visually bounded when shown in job detail. Large imported-record summaries should use pagination or an equivalent bounded native table pattern rather than an expanding wall of rows or div-only table imitation.
...
- Preview plan sections must be visually bounded and must state when only a subset of fields, assets, images, or messages is shown.
...
- Import history, detail, cancellation, and removal failures must use fixed action-specific recovery copy unless an adapter error is explicitly marked safe for users. Raw server, provider, storage, and transport messages must never be rendered in the import workspace or its transient surfaces.
- Durable failure detail must show the retained phase-specific diagnostic, what was already imported, what was not imported, and the next safe action. Technical identifiers may be available in an optional diagnostic detail, but the default failure summary must remain plain and actionable.
