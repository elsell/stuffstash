...
- Web clients must submit S3-compatible presigned POST direct uploads as multipart form data containing every adapter-provided policy field before the file part. Missing policy fields, wrong field order, wrong method, wrong content type, or rejected responses are Garage direct-upload failures.
- In the self-host Garage topology, a direct-upload transport failure must be surfaced as a configuration or upload failure. The web client must not silently treat a rejected Garage target as success, and the public self-host verification path must not rely on the JSON fallback to prove Garage media storage works.
...
- Creating an attachment, including direct-upload completion, must advance the owning asset's `updatedAt` timestamp in the same metadata transaction so updated-descending asset lists treat photo changes as asset changes.
